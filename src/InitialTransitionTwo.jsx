...
function InitialTransitionTwo({ onTransitionEnd }) {
  const [isTransitioning, setIsTransitioning] = useState(false);

  useEffect(() => {
    // Delay to keep teal background visible initially
    const timer = setTimeout(() => {
      setIsTransitioning(true);
      
      // Trigger main transition after animation completes
      setTimeout(onTransitionEnd, 2000); // 2 seconds for full transition
    }, 2500); // 2.5-second delay before transition
...
export default InitialTransitionTwo;
